...
const importData = async () => {
  try {
    ...
    const fertilizer1 = await Material.create({ farmProfile: farm._id, name: 'Phân NPK 16-16-8', type: 'FERTILIZER', unit: 'kg', quantity: 500, minQuantityAlert: 50 });
    const fertilizer2 = await Material.create({ farmProfile: farm._id, name: 'Phân Hữu cơ vi sinh', type: 'FERTILIZER', unit: 'kg', quantity: 1000, minQuantityAlert: 100 });
    const pesticide1 = await Material.create({ farmProfile: farm._id, name: 'Thuốc trừ sâu Radiant', type: 'PESTICIDE', unit: 'ml', quantity: 2000, minQuantityAlert: 200 });
    console.log(`Đã tạo dữ liệu kho vật tư (Materials)`);

    // 2. Tạo Bảng Nhật ký (Boards)
    const cBoard = await CultivationBoard.create({ farmProfile: farm._id, name: 'Khu A - Cà chua Thu Đông', cropType: 'Cà chua', areaSqm: 1000, startDate: new Date('2026-06-01'), status: 'ACTIVE' });
    const fBoard = await FertilizerBoard.create({ farmProfile: farm._id, name: 'Khu A - Bón phân Cà chua', cropType: 'Cà chua', areaSqm: 1000, startDate: new Date('2026-06-01'), status: 'ACTIVE' });
    const pBoard = await PesticideBoard.create({ farmProfile: farm._id, name: 'Khu A - Phun thuốc Cà chua', cropType: 'Cà chua', areaSqm: 1000, startDate: new Date('2026-06-01'), status: 'ACTIVE' });
    ...
    await Task.create({ farmProfile: farm._id, title: 'Tưới nước khu A', dueDate: today, status: 'PENDING', notes: 'Tưới đẫm vào buổi sáng' });
    await Task.create({ farmProfile: farm._id, title: 'Kiểm tra bọ trĩ', dueDate: tomorrow, status: 'PENDING' });
    await Task.create({ farmProfile: farm._id, title: 'Bón phân NPK đợt 3', dueDate: nextWeek, status: 'PENDING', notes: 'Chuẩn bị 30kg NPK' });
    await Task.create({ farmProfile: farm._id, title: 'Thu dọn cỏ dại', dueDate: today, status: 'COMPLETED' });
