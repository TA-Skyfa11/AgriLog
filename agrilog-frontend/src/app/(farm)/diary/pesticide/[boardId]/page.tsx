...
import { Trash2, Edit2, Calendar, FileText, ShieldAlert } from 'lucide-react';
...
export default function PesticideDetailPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        setFormData({ ...formData, materialId: '', quantity: '', phiDays: '', notes: '' });
        loadData();
      }
    } catch (error) {
  ...
  const handleDelete = async (entryId: string) => {
    ...
    try {
      const res = await fetchAPI(`/pesticide-boards/entries/${entryId}`, { method: 'DELETE' });
  ...
  if (loading) return <div>Đang tải dữ liệu bảng thuốc BVTV...</div>;
  if (!board) return <div>Không tìm thấy bảng thuốc BVTV</div>;

  const availableMaterials = materials.filter(m => m.type === 'PESTICIDE');
    ...
    <div className={styles.container}>
      ...
      <div className={styles.content}>
        <div className={styles.formCard}>
          <h2 className={styles.formCardTitle}>Thêm mới Nhật ký Phun thuốc</h2>
          <form onSubmit={handleSubmit}>
            ...
            <div className={styles.formGroup}>
              <label className={styles.label}>Tên Thuốc BVTV</label>
              ...
              <tbody>
                {entries.map(entry => (
                  <tr key={entry._id}>
                    ...
                    <td style={{ color: '#ea580c', fontWeight: 600 }}>{entry.materialName}</td>
                    <td>{entry.quantity} {entry.unit}</td>
                    <td>{entry.phiDays ? `${entry.phiDays} ngày` : '-'}</td>
