...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const loadProduct = async () => {
      try {
        ...
        if (res.success && res.data) {
          ...
          setCategory(p.category || 'FERTILIZER');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (imageMode === 'upload' && imageFile) {
        ...
        finalImages = [uploadData.imageUrl];
      } else if (imageMode === 'url' && imageUrl) {
        finalImages = [imageUrl];
      }

      if (finalImages.length === 0 && imagePreview) {
          // If no new upload and mode changed, keep existing preview
          finalImages = [imagePreview];
      }

      const token = localStorage.getItem('token');
      const resResponse = await fetch(`${API_URL}/products/${id}`, {
        method: 'PUT',
        headers: {
      ...
      if (!resResponse.ok) {
        throw new Error(res.message || 'Có lỗi xảy ra khi cập nhật sản phẩm');
      }
  ...
  if (loadingInit) return <div style={{ padding: '2rem', textAlign: 'center' }}>Đang tải...</div>;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', maxWidth: '700px', margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
        ...
        <div>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--color-text-main)' }}>Chỉnh sửa sản phẩm</h1>
          <p style={{ color: 'var(--color-text-muted)', fontSize: '0.875rem' }}>Chỉnh sửa thông tin sản phẩm và cập nhật lại hình ảnh.</p>
        </div>
      </div>
      ...
      <Card style={{ padding: '2rem' }}>
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          <div>
            ...
            <textarea style={{ ...inputStyle, minHeight: '100px', resize: 'vertical' as const }} value={description} onChange={e => setDescription(e.target.value)} placeholder="Mô tả chi tiết về sản phẩm..." required />
          </div>
          ...
          <button type="submit" disabled={loading || (!imagePreview && !imageFile && !imageUrl)} style={{
